# Quadratic-equations
Simple program with useful math features for solving quadratic equations like ax^2+bx+c=0
 //Hi, this my hurble code and it helps to solve quadratic equatoins like ax^2 + bx + c
#include <iostream>
int main()
{
	int a, b, c, d;
	...
	if (a != 0) {
		...
		d = ((b * b) - (4 * a * c));
		if (d < 0) {
			std::cout << ("Discriminant is less than 0, no roots");                    //So, the same tings happen
		}
		else {
			x = (-b + (sqrt(d))) / (2 * a);
			y = (-b - (sqrt(d))) / (2 * a);
